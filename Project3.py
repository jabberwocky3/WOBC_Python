...
def password_check(testString):
    ...
    Row4Upper = re.findall("[A-Z]{4}", testString)
    if len(Row4Upper) > 0:
        test = False

    Row4Lower = re.findall("[a-z]{4}", testString)
    if len(Row4Lower) > 0:
        test = False

    Row4Number = re.findall("[0-9]{4}", testString)
    if len(Row4Number) > 0:
        test = False

    Row4Special = re.findall("[^a-zA-Z0-9]{4}", testString)
